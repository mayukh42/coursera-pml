...
The original HAR data consisted of 19622 observations of 160 variables. For 4 locations, 3 Euler angles per location, 1 total acceleration measurement and its variance per location, 3 sensors per location, 3 dimensional readings per sensor, and 8 derived measurements per Euler angle accounted for 4 x (3 + 1+1 + 3x3 + 8x3) = 152 features. Row number, user name, 3 timestamp measurements, 2 window measurements, and activity classe made up for the remaining 8 features. 

All the derived variables (96+4) had missing data in them, hence were removed for analysis, reducing the features to 60. Further, features like row number (1), user name (1), timestamps (3), and window measurements (2) were assumed to be unrelated to the prediction problem. When the problem is to predict the type of activity from sensor readings, it should not matter which person has equipped the sensors or when the activity was performed. Hence these 7 features were removed. The final cleaned data therefore had 53 features. The first 52 columns contained predictor variables, and the 53rd column contained the outcome variable.

Data normalization was not done, since the machine learning algorithm used was Random Forests which do not require centering or scaling of data. 

For the structure of data after cleaning and the complete list of original features, refer Appendix at the end. 

**Learning from data**

The **Random Forest** algorithm **(R package: randomForest, function: randomForest)** was used to learn the parameters of the 52 predictor variables. Random Forests are very accurate (< 1% Out Of Bag Error Rate) without significant performance overhead, even if a high number of classification trees (~ 500) are used. However, they can be susceptible to overfitting, hence we partitioned the training data further into training and cross valication (CV) sets. 
...
As expected, the error rates reduce with higher p and nTree. The lowest error rate for CV set occurs at p=0.7, ntree=300. But from the trend, it could be a one-off incident. Indeed, running the RandomForest algorithm with 70% training data revealed ~ 0.5% error rate on the remaining 30% data (CV set), when the randomForest fitted object was used to predict on that data. Ignoring this slump, we thus conclude that an OOB error rate of ~ 0.5% is what we expect, if we use 0.7 as the proportion of training data, and this rate is not improved by reducing the number of trees from default value of 500, set in the randomForest function. Hence we go ahead with p=0.7 and ntree=500 for our learning. 
...
Hence, if we use this fit object on unseen data, then we expect the error rate to be 0.0045134 = 0.45%. Using this fit object, we predict the outcome in CV set. Below is the contingency table of the predicted outcomes (rows) with the actual labels (columns) in CV set.
...
Thus, we get roughly the same error rate (0.005607477 = 0.56%) for CV set as we had obtained earlier while conducting the iterations. This is however, larger than our expected error rate of 0.45%, but nonetheless, is not too far off. The analyses are paired by using the same seed for random number generator.
